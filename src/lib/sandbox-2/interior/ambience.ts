...
export function levelsAt(x: number, z: number, indoors: boolean, water: { x: number; z: number }[], herds: Partial<Record<'hens' | 'geese' | 'goats' | 'frogs' | 'bees', Where>>) {
	...
	return {
		...
		bees: herds.bees ? nearness(nearest(herds.bees()), 1.5, 14) : 0
	}
}
