---
title: Democracy is five things
originalTitle: "I don't get democracy"
source: https://www.youtube.com/watch?v=O7LvhSQ5Te4
type: video
author: Johnny Harris
authorUrl: https://www.youtube.com/@johnnyharris
added: 2026-09-14
categories: [civic, coop]
hook: >-
  Asked what democracy requires, people answer free elections — then gender equality, civil
  rights, taxing the rich. The word has come to mean "good government". Five dials fix that.
shift:
  from: Democracy is one thing — voting for your leaders.
  to: Democracy is five dials, and a country can sit high on some and low on others.
quote: "It means the wolves cannot vote to eat the sheep."
quoteBy: Adam, in the video
---

## The story

Ask people what democracy requires and they say free elections — then gender equality, civil rights, state aid for the unemployed, taxing the rich.

That's from a 2024 study of what ordinary citizens actually mean by the word, and it explains the confusion perfectly: most people use "democracy" as the name for whatever they think good government is.

Which is a problem, because the word is doing serious work right now. It's used to say democracy is in trouble, and to say the US was never supposed to be one, and those two sentences aren't even about the same thing.

The video borrows a sharper frame: V-Dem's five principles, five answers to one question — what does it mean for the people to rule?

They rule by **choosing who rules**. Madison called that a republic rather than a democracy; today it's electoral democracy, the thin base of the definition. Elections only count when losing is possible — in Russia the serious opponents are in prison, in exile or dead — and when suffrage is broad and the press is free enough to make the choice real.

They rule by **staying free**. Liberal democracy: rights the state cannot violate, rule of law, checks that constrain power between elections so you don't end up with an elected dictator. The wolves cannot vote to eat the sheep. And "liberal" here means the early-1800s idea of limiting government, not anything left-wing — Australia's and Canada's Liberal parties split on exactly that lineage.

They rule by **having equal access to power**. Egalitarian democracy: one vote each counts for little when most people give nothing and a few donors give hundreds of millions, when your employer can fire you for your opinions, or when you have no roof, no food and no time.

They rule by **taking part between elections**. Participatory democracy: protest, unions, school boards, city councils, referendums — the places where ordinary people actually get input.

Participation without understanding is noise, which is where the "voters are idiots" argument usually wins. Except the famous study says something subtler: asked whether the 1975 Public Affairs Act should be repealed, two-thirds had no opinion and a third answered anyway — about a law that never existed.

The fifth: they rule by **reasoning together**. Ireland handed its abortion deadlock to 99 randomly selected citizens, who heard evidence in public across weekends, questioned experts, argued, and recommended. They still didn't agree — but the country had held the conversation in miniature, then voted, and a deadlock its politics couldn't touch was settled legitimately.

The five don't always cooperate: maximise participation and the meetings fill with whoever has time to spare, which costs you equality.

So no country is simply a democracy or not. Each one sits high on some of these dials and low on others — and the useful question stops being "is this a democracy?" and becomes "which of the five are we failing at?".

## Beliefs that shift

- *Democracy is a system of government.* → Democracy is the word most people use for whatever they consider good government.
- *Elections make a democracy.* → Elections make a democracy only when losing is possible and the press is free.
- *One person one vote means equal power.* → One person one vote plus unlimited donations means deeply unequal power.

## The five principles

| Principle         | The people rule by…            | What it needs                                             | Where it breaks                              |
| ----------------- | ------------------------------ | --------------------------------------------------------- | -------------------------------------------- |
| **Electoral**     | choosing who rules             | real competition, broad suffrage, free press              | elections without a genuine choice           |
| **Liberal**       | staying free                   | rights, rule of law, checks between elections             | the elected leader with no limits            |
| **Egalitarian**   | having equal access to power   | needs met, equal influence, protection from your employer | super PACs; inequality becoming oligarchy    |
| **Participatory** | taking part between elections  | protest, unions, town halls, referendums                  | only those with spare time show up           |
| **Deliberative**  | reasoning together             | listening, expert input, a public process                 | politics as a debate somebody has to win     |

## What we learn

- **"Liberal" here has nothing to do with left-wing.** It's the 1800s meaning: limits on state power. Australia's Liberal Party runs on free enterprise; Canada's on a just society — same word, split lineage.
- **A citizens' assembly recommends; it doesn't decide.** Ireland's 99 broke the deadlock, and the referendum settled it.
- **Whoever convenes the assembly picks the experts.** That's its real vulnerability, and it doesn't disappear by ignoring it.
- **Treat uninformed opinion as noise, not data.** Two-thirds said they had no view on the (invented) 1975 Public Affairs Act. A third answered anyway.
- **Deliberation isn't about flipping people.** It's about being able to justify your position — and understanding why the other side holds theirs.
- **The dials trade against each other.** Maximise participation and you can weaken equality. There's no configuration that maxes all five.
- **Check the quote before you repeat it.** Franklin's wolves and Churchill's five-minute conversation are both invented — 1990 and 1992, respectively.

## Open questions

- Where does maiaCITY actually sit on each of the five today — not where we'd like to?
- Is sortition a replacement for elections or only a complement? The video leaves this genuinely open.
- How do we stop deliberation being captured by whoever frames the question?
- What's the smallest scale at which a citizens' assembly is worth its cost?
